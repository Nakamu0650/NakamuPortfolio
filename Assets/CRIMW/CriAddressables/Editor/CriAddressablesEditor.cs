	...
	internal class CriAddressablesEditor : IPreprocessBuildWithReport
	{
		...
		class ModificationHook : UnityEditor.AssetModificationProcessor
		{
			private static AssetDeleteResult OnWillDeleteAsset(string assetPath, RemoveAssetOptions options)
			{
				...
				if (!(impl is CriAddressableAssetImpl) && !(impl is CriLocalAddressablesAssetImpl)) return AssetDeleteResult.DidNotDelete;
		...
		internal static CriLocalAddressablesAnchor GetLocalAnchor() => AssetDatabase.LoadAssetAtPath<CriLocalAddressablesAnchor>(AssetDatabase.GUIDToAssetPath(AssetDatabase.FindAssets($"t:{nameof(CriLocalAddressablesAnchor)} LocalAnchor").First()));
	}
